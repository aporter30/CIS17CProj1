 ...
 * Created on April 5th, 2024, 12:36 PM
 * Purpose:  Project 1 - A simple Blackjack game
 */
...
int main() {
    ...
    while (playerBalance > 0) {
        ...
        playerHand.sort(Deck::compareCards);

        // Sort the dealer's hand
        dealerHand.sort(Deck::compareCards);
...
bool Deck::compareCards(const Card& card1, const Card& card2) {
    // Assuming Ace has the lowest value
...
Deck::Deck() {
    ...
    // Deal another card to the dealer, ensuring it's not the same as the player's card
    while (true) {
        ...
        // If the dealer's card matches the player's card, shuffle the deck and try again
        shuffleDeck();
    }
}
    ...
    void Deck::updateRemainingCards(const Card& card) {
        remainingCards[card.rank + card.suit]--; // Decrease count of dealt card
    }
    ...
    do {
        ...
        if (choice == 'h') {
            ...
            if (handValue > 21) {
                ...
                break; // End the turn if the hand value exceeds 21
            }
        }
    } while (choice == 'h');
}
    ...
    // Keep hitting until the hand value is 17 or higher
    while ((this->*calculateHandValue)(dealerHand) < 17) {
        ...
        // Break the loop after hitting once
        break;
    }
}
    ...
    // Calculate the sum of all non-Ace cards
    ...
    for (int i = 0; i < numAces; ++i) {
        if (sum + 11 <= 21) {
            sum += 11; // If adding 11 doesn't bust the hand, count Ace as 11
        } else {
            sum += 1; // Otherwise, count Ace as 1
        }
    }
